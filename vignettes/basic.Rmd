...
module, it just means `module_search()` can find them.
...
Dockerfiles, see the [Docker docs](https://docs.docker.com/engine/docker-overview/).
...
Using our Dockerfile that has been preloaded with the installed R pacakge, we
can now build our Docker image using `module_build()`.
